...
quint32 gvcpReceiveAck(QUdpSocket& udpSock, strNonStdGvcpAckHdr& ackHeader) {
    ...
    /* Receive datagram. */
    error = CamEthernetInterface::camEthernetReceiveAck(udpSock, datagram);
    ...
    if ((error == false) && (ackHeader.genericAckHdr.length > 0) && tempArray.length()) {
        ...
        qDebug() << "(" << __FILENAME__ << ":" << __LINE__ << ")" << "Command specific data in an ack cannot be empty";
        error = true;
    }
...
quint32 gvcpSendCmd(QUdpSocket& udpSock, const quint16 cmdType, const QByteArray &cmdSpecificData,
    ...
    if (!error) {
        ...
        error = CamEthernetInterface::camEthernetSendCmd(udpSock, datagram);
    }
