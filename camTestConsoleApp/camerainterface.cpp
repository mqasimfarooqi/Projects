...
void camMakeGenericHeader(const quint16 cmdType, QByteArray& datagram,
...
bool CameraInterface::camSendCmd(QUdpSocket *udpSock, const quint16 cmdType,
    ...
    if (!error) {
        ...
        camMakeGenericHeader(cmdType, datagram, cmdSpecificData, reqId);
...
bool CameraInterface::camReceiveAck(QUdpSocket *udpSock, strNonStdGvcpAckHdr& ackHeader) {
    ...
    if (!error && (ackHeader.genericAckHdr.length > 0) && tempArray.length()) {
        ...
        case GVCP_READMEM_ACK:
            ...
            /* Copy address. */
            memcpy(&readMemAck->address, tempArray.data() + sizeof(ackHeader.genericAckHdr),
                   sizeof(readMemAck->address));
        ...
        case GVCP_READREG_ACK:
            ...
            readRegAck->registerData.append(tempArray.data() + sizeof(ackHeader.genericAckHdr),
                                            ackHeader.genericAckHdr.length);
    ...
    if (error) {
        ...
        if (ackHeader.cmdSpecificAckHdr) {
            ...
            /* Free the required memory inc ase of error. */
